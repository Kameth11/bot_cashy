...
- **`sanitizarInput` ya no borra apóstrofes ni comillas.** Borraba
  `<>"'&\`` directo, así que `"D'Amato"` quedaba `"DAmato"`. Ninguno de
  esos caracteres es peligroso hoy en este código: no hay ningún render
  HTML (React escapa JSX solo, nadie usa `dangerouslySetInnerHTML`) ni
  `parse_mode: 'HTML'` en Telegram (todo es Markdown), y la inyección de
  fórmulas de Sheets ya la cubre el prefijo `'` cuando el texto empieza con
  `=+-@` — nada de esto depende de haber borrado esos caracteres. Se dejan
  pasar tal cual. El backtick (`` ` ``) también se deja pasar por
  consistencia con `_`, `*`, `(`, `)`, `[`, `]` (los otros caracteres
  especiales de Markdown, que igual nunca se borraban acá) — la protección
  real contra Markdown roto en los mensajes de Telegram es
  responsabilidad de `escapeMarkdown` al armar cada mensaje, no de esta
